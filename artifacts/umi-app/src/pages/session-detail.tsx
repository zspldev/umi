          ...
          return (
            <div key={turn.id} className={`flex flex-col max-w-[85%] ${!isSpeakerOne ? 'self-end items-end' : 'self-start items-start'}`}>
              ...
              <div className={`p-4 rounded-2xl ${!isSpeakerOne ? 'bg-primary text-white rounded-tr-sm' : 'bg-white shadow-sm border border-muted/50 text-secondary rounded-tl-sm'}`}>
                <p className={`text-sm opacity-70 mb-2 pb-2 border-b ${!isSpeakerOne ? 'border-white/20' : 'border-muted'} ${['hi', 'mr'].includes(lang) ? "font-devanagari" : ""}`}>
                  {turn.original}
                </p>
                <p className={`text-base font-medium ${['hi', 'mr'].includes(isSpeakerOne ? session.speakerTwoLang : session.speakerOneLang) ? "font-devanagari" : ""}`}>
                  {turn.translated}
                </p>
              </div>
            </div>
